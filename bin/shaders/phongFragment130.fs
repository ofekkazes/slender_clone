...
vec4 calcSpotLights(SpotLight spotLight, vec3 normal)
{
    ...
    {
        color = calcPointLights(spotLight.pointLight, normal) * 
...
void main()
{
    ...
    for(int i=0; i < MAX_POINT_LIGHTS; i++)
        if(pointLights[i].base.intensity > 0)
            totalLight += calcPointLights(pointLights[i], normal);

    for(int i=0; i < MAX_SPOT_LIGHTS; i++)
        if(spotLights[i].pointLight.base.intensity > 0)
            totalLight += calcSpotLights(spotLights[i], normal);
